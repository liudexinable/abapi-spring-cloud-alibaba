...
public class BankUtil {
    ...
    public static Map<String, String> banks() {
        Map<String, String> map = JSON.parseObject(BANK_STR, Map.class);
        return map;
    }
     ...
     * @return
     */
    public static String bankName(String bank) {
        Map<String, String> map = banks();
        return map.get(bank);
    }
     ...
     * stat
     */
    public static Map validateAndCacheCardInfo(String cardNo) {
    ...
    /**
     * 方法描述 隐藏银行卡号中间的字符串（使用*号），显示前四后四位
     *
     * @param cardNo
     * @return
     */
    public static String hideCardNo2(String cardNo) {
        if (StringUtils.isBlank(cardNo)) {
            return cardNo;
        }
        int length = cardNo.length();
     ...
     * @return
     */
    public static String getCardNoLastFour(String cardNo) {
        if (StringUtils.isBlank(cardNo)) {
            return cardNo;
        }
        int length = cardNo.length();
        ...
        if (length > beforeLength) {
            cardNo = cardNo.substring(length - 4);
        }
        return cardNo;
    }
