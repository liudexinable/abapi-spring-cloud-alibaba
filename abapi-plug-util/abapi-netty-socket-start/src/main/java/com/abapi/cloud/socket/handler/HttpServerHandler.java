...
public class HttpServerHandler extends SimpleChannelInboundHandler<FullHttpRequest> {
    ...
    private final NettyWebsocketConfigProperties config;
    ...
    public HttpServerHandler(NettyWebsocketConfigProperties config,WebsocketEndpointServiceMapping websocketEndpointServiceMapping) {
    ...
    private void handleHttpRequest(ChannelHandlerContext ctx, FullHttpRequest req) {
        ...
        if (handshaker == null) {
            ...
            if (config.getReaderIdleTime() != 0 || config.getWriterIdleTime() != 0 || config.getAllIdleTime() != 0) {
                pipeline.addLast(new IdleStateHandler(config.getReaderIdleTime(), config.getWriterIdleTime(), config.getAllIdleTime()));
            }
            if (config.getUseCompressionHandler()) {
                pipeline.addLast(new WebSocketServerCompressionHandler());
            }
            pipeline.addLast(new WebsccoketServiceHandler(websocketEndpointServiceMapping));
            final String finalPath = path;
            final String finalOriginalParam = originalParam;
            handshaker.handshake(channel, req).addListener(future -> {
                if (future.isSuccess()) {
                    websocketEndpointServiceMapping.doOnOpen(ctx, req, finalPath, finalOriginalParam);
                } else {
